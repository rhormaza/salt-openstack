    ...
    def _check_ovs_version(self):
        if p_const.TYPE_VXLAN in self.tunnel_types:
            try:
                ...
                # FIXME(raul): hack to prevent to raise the exception
                # we know vxlan works in Centos6 so we can do this.
                # Long term fix is find out that upstream fixes this!
                #LOG.exception(_("Agent terminated"))
                #raise SystemExit(1)
                LOG.warning(_("byRaul: VXLAN check failed but we know kernel and ovs are OK. FIX *properly*"))

...
def endpoint_create(region, service_id, publicurl, adminurl, 
                internalurl, profile=None, **connection_args):
    '''
    Create an endpoint (keystone endpoint-create)

    CLI Examples:

    .. code-block:: bash

        salt '*' keystone.endpoint_create name=jack password=zero email=jack@halloweentown.org tenant_id=a28a7b5a999a455f84b1f5210264375e enabled=True
    '''
    kstone = auth(profile, **connection_args)
    endpoint = kstone.endpoints.create(region=region,
...
def endpoint_delete(id,  profile=None, **connection_args):
    '''
    Delete an endpoint (keystone endpoint-delete)

    CLI Examples:

    .. code-block:: bash

        salt '*' keystone.endpoint_delete c965f79c4f864eaaa9c3b41904e67082
        salt '*' keystone.endpoint_delete id=c965f79c4f864eaaa9c3b41904e67082
    '''
    kstone = auth(profile, **connection_args)
    endpoint = kstone.endpoints.delete(id)
    return 'Endpoint ID {0} deleted'.format(id)

def endpoint_getget(service, profile=None, **connection_args):
    '''
...
def endpoint_exact_get(service, region=None, publicurl=None, adminurl=None, 
                internalurl=None, profile=None, **connection_args):
    '''
    ...
    for endpoint in kstone.endpoints.list():
        if endpoint.service_id == service:
            ret = {'id': endpoint.id,
                   'region': endpoint.region,
                   'adminurl': endpoint.adminurl,
                   'internalurl': endpoint.internalurl,
                   ...
                   'service_id': endpoint.service_id}
            return ret
    return {'Error': 'Unable to find endpoint for service id'}

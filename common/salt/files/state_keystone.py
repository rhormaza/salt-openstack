...
def service_present(name, service_type, description=None, endpoints=None,
    ...
    '''
    logger.debug('>> About to create service: %s', name)
    ret = {'name': name,
           'changes': {},
           'result': True,
    ...
    if 'Error' not in role:
        if endpoints:
            logger.debug('>> Searching for endpoints: %s with role %s', endpoints, role[name])
            e = __salt__['keystone.endpoint_get'](service=role[name]['id'],
                                                 profile=profile,
                                                 **connection_args)
            logger.debug('>> Endpoint search result: %s', e)
            if 'Error' in e:
                # This means endpoint does not exist
                for endpt in endpoints:
                    e = __salt__['keystone.endpoint_create'](
                                            region=endpt['region'],
                                            service_id=role[name]['id'],
                                            adminurl=endpt['adminurl'],
                                            internalurl=endpt['internalurl'],
                                            publicurl=endpt['publicurl'],
                                            profile=profile,
                                            **connection_args)
                logger.debug('>> Endpoint %s has been created', e)
                
        return ret
    else:
        # Create service
        role = __salt__['keystone.service_create'](name, service_type,
                                                   description,
                                                   profile=profile,
                                                   **connection_args)
        if endpoints:
